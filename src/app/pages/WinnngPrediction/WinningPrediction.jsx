...
export const WinningPrediction = () => {
  ...
  const [PannaDigits,setPannaDigits]=React.useState([]);
...
const SubmitData =async(Date)=>{
  ...
  formData.append("panna",formValues.panna)
  try {
    // setIsLoading(true);
    const response = await axios.post(
      `${Base_Url2}bid_win`,
...
React.useEffect(()=>{
  ...
  return (
    ...
    <Card sx={{marginBottom:"30px"}}>
        ...
        <Box>
      <Box>
        <Typography variant='h6'>Win Pridiction</Typography>
      </Box>
      ...
      <Grid item xs={12} sm={2} md={2} >
        ...
        <FormControl fullWidth>
          ...
          <Select
             ...
             id="demo-simple-select"
             name="panna"
             value={formValues.panna}
             onChange={handleChange}
            
          >
          {
             PannaDigits.map((el,index)=>{
              return (
      ...
      <Box>
    <Box>
      <Typography variant='h6'>WINNING PREDICTION LIST</Typography>
    </Box>
    <Box sx={{marginTop:"30px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
    
         <Box sx={{display:"flex"}}>
        <Button  sx={{marginLeft:"20px"}} variant="contained" ><StarIcon sx={{marginRight:"10px"}} />Total Bid Amount {totalBidAmount}</Button>
      <Button sx={{marginLeft:"20px"}} variant="outlined"> <WineBarIcon sx={{marginRight:"10px"}} />Total Winning Amount {WinningAmount}</Button>
        </Box>
